  ...
  useEffect(() => {
    ...
    const kd = (e: KeyboardEvent) => {
      ...
      if (k === "i") {
        ...
        setShowTalents((v) => !v);
        e.preventDefault();
      } else if (g.devMode && e.shiftKey && /^[0-9]$/.test(k)) {
        // Shift+1..9 → biomas 1..9; Shift+0 → bioma 10
        const biomeId = k === "0" ? 10 : parseInt(k, 10);
        devTeleportToBiome(g, biomeId);
        e.preventDefault();
      } else if (g.devMode && (k === "[" || k === "]")) {
        // [ / ] navegam blocos de 10 biomas (11-20, 21-30, ...)
        const cur = g.activeCaveBiome ?? 1;
        const target = k === "]" ? Math.min(80, cur + 10) : Math.max(1, cur - 10);
        devTeleportToBiome(g, target);
        e.preventDefault();
      } else if (k === "escape") {
        g.panel = "none";
...
// ============ Dev Mode HUD ============
function DevModeHud({ stateRef }: { stateRef: React.MutableRefObject<GameState | null> }) {
  const [, tick] = useState(0);
  useEffect(() => {
    const id = window.setInterval(() => tick((n) => (n + 1) & 0xffff), 250);
    return () => window.clearInterval(id);
  }, []);
  const g = stateRef.current;
  if (!g || !g.devMode) return null;
  return (
    <div className="fixed top-2 right-2 z-30 pointer-events-none">
      <div className="bg-black/80 border border-yellow-400 text-yellow-200 px-3 py-2 text-xs font-mono shadow-lg">
        <div className="text-yellow-400 font-bold tracking-widest">⚡ MODO DEV</div>
        <div>Imortal · Lv 1000</div>
        <div className="text-[10px] opacity-70 mt-1">P: sair · Shift+1-9,0: bioma 1-10</div>
        <div className="text-[10px] opacity-70">[ / ]: -10 / +10 biomas</div>
        <div className="text-[10px] opacity-50 mt-1">Não é salvo.</div>
      </div>
    </div>
  );
}
