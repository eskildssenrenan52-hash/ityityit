...
export type GameState = {
  ...
  hitstopTime?: number;
  // Dev mode (tecla P). Nunca é salvo. Player fica imortal, lv 1000, com HUD indicando.
  devMode?: boolean;
  _devPrevStats?: {
    level: number;
    maxHp: number;
    ...
    maxMp: number;
    mp: number;
    attr: { str: number; agi: number; int: number; vit: number };
  };
};
...
function damageEnemy(g: GameState, e: Enemy, dmg: number) {
  ...
  if (mods.some(m => m.id === "cristalizado")) {
    ...
    if (!g.devMode) g.player.hp -= reflected;
...
function damagePlayer(g: GameState, dmg: number, source?: Enemy) {
  const p = g.player;
  if (g.devMode) return; // imortal em Modo Dev
...
export function save(g: GameState) {
  // Se em Modo Dev, salva usando os stats originais (pré-dev) para não persistir imortalidade/lv 1000.
  const s = g._devPrevStats;
  const inDev = !!g.devMode && !!s;
  const pl = g.player;
  const data = {
    seed: g.world.seed,
    player: {
      ...
      pos: pl.pos,
      hp: inDev ? s!.hp : pl.hp,
      maxHp: inDev ? s!.maxHp : pl.maxHp,
      mp: inDev ? s!.mp : pl.mp,
      maxMp: inDev ? s!.maxMp : pl.maxMp,
      level: inDev ? s!.level : pl.level,
      xp: pl.xp, xpNext: pl.xpNext,
      attr: inDev ? s!.attr : pl.attr,
      attrPoints: pl.attrPoints, gold: pl.gold,
      inventory: pl.inventory, equipped: pl.equipped,
      talents: pl.talents || {},
      talentPoints: pl.talentPoints || 0,
    },
...
// ===== Dev Mode (tecla P) — nunca é persistido =====
export function toggleDevMode(g: GameState) {
  if (!g.devMode) {
    g._devPrevStats = {
    ...
    g.devMode = true;
    g.player.level = 1000;
    g.player.attr = { str: 999, agi: 999, int: 999, vit: 999 };
    g.player.maxHp = 999999;
    g.player.hp = 999999;
    g.player.maxMp = 999999;
    g.player.mp = 999999;
    logMsg(g, "⚡ MODO DEV ativado — imortal, lv 1000. NÃO será salvo.", "#ffdd44");
  } else {
    const s = g._devPrevStats;
    if (s) {
      g.player.level = s.level;
      g.player.attr = s.attr;
      g.player.maxHp = s.maxHp;
      g.player.hp = Math.min(s.maxHp, s.hp);
      g.player.maxMp = s.maxMp;
      g.player.mp = Math.min(s.maxMp, s.mp);
    }
    g.devMode = false;
    g._devPrevStats = undefined;
    logMsg(g, "Modo Dev desativado. Stats restaurados.", "#f0c060");
  }
}

// Teleporta o player para o overworld na entrada (escada) do bioma indicado.
export function devTeleportToBiome(g: GameState, biomeId: number): boolean {
  if (!g.devMode) return false;
  // Sai de qualquer masmorra/arena primeiro
  if (g.activeCaveBiome != null || g.activeGrandFloor > 0 || g.activeArena) {
    snapshotFloor(g);
    g.activeMap = g.world;
    g.activeCaveBiome = null;
    g.activeFloor = 1;
  ...
  if (stair) {
    ...
    // Fallback: busca tile do bioma no overworld
    const w = g.world;
    let found: { x: number; y: number } | null = null;
    outer: for (let ty = 0; ty < w.size; ty += 4) {
      for (let tx = 0; tx < w.size; tx += 4) {
        const t = w.get(tx, ty);
        if (t && t.biome === biomeId && !w.isBlocked(tx, ty)) {
          found = { x: tx * TILE + TILE / 2, y: ty * TILE + TILE / 2 };
          break outer;
        }
      }
    }
    if (found) { g.player.pos.x = found.x; g.player.pos.y = found.y; }
    else { logMsg(g, `Bioma ${biomeId} não encontrado no mapa.`, "#ff8060"); return false; }
  }
  g.camera.x = g.player.pos.x;
  g.camera.y = g.player.pos.y;
  g.stairCd = 1.2;
  const b = BIOMES.find(bb => bb.id === biomeId);
  logMsg(g, `[DEV] Teleporte → ${b?.name ?? "bioma " + biomeId}.`, "#ffdd44");
  return true;
}
